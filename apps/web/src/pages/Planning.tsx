...
import { SessionCard } from '../components/sm'
import { useSmPrdSessions, useSmSessions } from '../lib/api-client'
...
export function PlanningPage() {
	...
	const { data, error, isLoading, mutate } = useSmSessions()
	const { data: prdSessions } = useSmPrdSessions()
	...
	const sessions = data ?? []
	const availablePrds = prdSessions ?? []
	...
	const handleCreateSession = async () => {
		...
		try {
			...
			const response = await fetch(`${API_URL}/api/v1/sm/sessions`, {
				method: 'POST',
				headers: {
			...
			navigate(`/requisitos/${result.data.id}`)
		} catch (_err) {
		} finally {
			setIsCreating(false)
		}
					...
					>
						<div>
							...
							<p className="text-lg text-muted-foreground">
								Planeje sprints e entregas com epics e user stories.
							</p>
						</div>
						...
						<div className="mb-8 grid gap-4 sm:grid-cols-3">
							...
							>
								<div className="flex items-center gap-4">
									...
									>
										<FolderOpen className="h-5 w-5" style={{ color: '#22c55e' }} />
									</div>
									<div>
										<p className="text-2xl font-bold text-foreground">{stats.total}</p>
										<p className="text-sm text-muted-foreground">Total de Planejamentos</p>
									</div>
								</div>
							</div>
						...
						>
							<div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-white py-16 text-center">
								...
								<h2 className="mb-2 text-xl font-semibold text-foreground">
									Comece seu primeiro planejamento
								</h2>
								<p className="mx-auto mb-6 max-w-md text-muted-foreground">
									Crie epics e user stories para organizar o desenvolvimento. A Anna vai guiar você
									em cada etapa.
								</p>
