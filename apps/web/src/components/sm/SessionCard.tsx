import type { SmSession, SmStatus, SmStep } from '@repo/shared'
import { Calendar, Clock, FileText, LayoutList } from 'lucide-react'
...
const statusConfig: Record<SmStatus, { label: string; color: string }> = {
...
const stepLabels: Record<SmStep, string> = {
	init: 'Inicializacao',
	epics: 'Definicao de Epics',
	stories: 'Criacao de Stories',
...
function getProgress(stepsCompleted: SmStep[], currentStep: SmStep, sessionStatus: SmStatus): number {
...
export function SessionCard({ session }: SessionCardProps) {
	...
	return (
		<Link href={`/requisitos/${session.id}`}>
			<div className="group h-full cursor-pointer">
				<div className="flex h-full flex-col rounded-xl border border-border bg-white p-6 transition-colors hover:border-primary/30">
					...
					<div className="mb-3 flex flex-wrap gap-1.5">
						{session.totalEpics > 0 && (
							<span className="flex items-center gap-1 rounded-md bg-green-50 px-2 py-0.5 text-xs font-medium text-green-600">
								<LayoutList className="h-3 w-3" />
								{session.totalEpics} epics
							</span>
						)}
						{session.totalStories > 0 && (
							<span className="flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600">
								<FileText className="h-3 w-3" />
								{session.totalStories} stories
							</span>
						)}
						{session.totalStoryPoints > 0 && (
							<span className="rounded-md bg-[#f9fafb] px-2 py-0.5 text-xs font-medium text-muted-foreground">
								{session.totalStoryPoints} pts
							</span>
						)}
						{session.prdSessionId && (
